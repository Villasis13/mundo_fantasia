...
class ReporteStockMinimo extends Component
{
    ...
    {
        abort_if(!auth()->user()->can('reporte_stock_minimo.listar'), 403);
    ...
    {
        if (!auth()->user()->can('reporte_stock_minimo.exportar')) { session()->flash('error', 'Sin permiso para exportar.'); return; }
        $this->dispatch('abrirEnlaces', url: route('reporte.stock_minimo_pdf', $this->buildParams()));
    }

    public function imprimirExcel(): void
    {
        if (!auth()->user()->can('reporte_stock_minimo.exportar')) { session()->flash('error', 'Sin permiso para exportar.'); return; }
        $this->dispatch('abrirEnlaces', url: route('reporte.stock_minimo_excel', $this->buildParams()));
    }
    ...
    private function buildQuery()
    {
        ...
        return $query->select(
            ...
            'ps.ps_stock', 'ps.ps_stock_minimo', 't.tienda_nombre',
            DB::raw('(ps.ps_stock_minimo - ps.ps_stock) as faltante')
    ...
    public function render()
    {
        ...
        $filas = collect();
        $totalAlertas = 0; $sinStock = 0;

        if ($this->buscado) {
            $filas = $this->buildQuery()->paginate($this->porPagina);
            $all   = $this->buildQuery()->get();
            $totalAlertas = $all->count();
            $sinStock     = $all->where('ps_stock', '<=', 0)->count();
        }

        return view('livewire.reporte.reporte-stock-minimo', compact(
            'esSuperAdmin', 'esAdmin', 'empresas', 'filas', 'totalAlertas', 'sinStock'
        ));
    }
}
